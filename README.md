<div align="center">

<!-- Logo/Banner Area -->
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=70&duration=2000&pause=1000&color=D3FD50&center=true&vCenter=true&multiline=false&repeat=true&width=600&height=100&lines=K72" alt="K72" />

<h3>
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=400&size=22&duration=4000&pause=1000&color=FFFFFF&center=true&vCenter=true&multiline=false&repeat=true&width=435&lines=Creative+Agency+Portfolio;Powered+by+React+%26+GSAP;Smooth+Animations+%26+Effects" alt="Typing SVG" />
</h3>

<!-- Badges -->
<p align="center">
  <img src="https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB" alt="React" />
  <img src="https://img.shields.io/badge/Vite-B73BFE?style=for-the-badge&logo=vite&logoColor=FFD62E" alt="Vite" />
  <img src="https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white" alt="Tailwind" />
  <img src="https://img.shields.io/badge/GSAP-2C2C2C?style=for-the-badge&logo=greensock&logoColor=88CE02" alt="GSAP" />
  <img src="https://img.shields.io/badge/React_Router-CA4245?style=for-the-badge&logo=react-router&logoColor=white" alt="React Router" />
</p>

<!-- Quick Links -->
<p align="center">
  <a href="#-features"><strong>Features</strong></a> •
  <a href="#-quick-start"><strong>Quick Start</strong></a> •
  <a href="#-tech-stack"><strong>Tech Stack</strong></a> •
  <a href="#-showcase"><strong>Showcase</strong></a> •
  <a href="#-structure"><strong>Structure</strong></a>
</p>

<br />

<!-- Decorative Line -->
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" alt="line">

</div>

<br />

## ✨ Features

```
🎬  SILKY ANIMATIONS
    ⚡ GSAP-powered transitions
    🎪 ScrollTrigger magic
    🌊 Smooth page routing
    🎭 Cinematic stair effects
    📜 Frame-by-frame sequences
    🎯 Interactive micro-interactions

🚀  BLAZING PERFORMANCE
    🖼️ Lazy-loaded images
    ⚙️ Smart code splitting
    📦 WebP compression
    🎥 Optimized video loading
    ⚡ Lightning-fast builds
    🎯 Sub-second page loads

💎  MODERN DESIGN
    📱 Fully responsive
    🎨 Polished UI/UX
    🌈 Custom animations
    ✨ Interactive elements
    🎯 Pixel-perfect layouts
    🖌️ Typography excellence

🛠️  DEV EXPERIENCE
    🔥 HMR enabled
    📝 ESLint configured
    🎯 React 19 features
    🧩 Component-based
    📐 Clean architecture
    🔧 Easy customization
```

<br />

<div align="center">
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" alt="line">
</div>

<br />

## 📖 Pages Showcase

<div align="center">

| 🎯 Page | 📝 Description | ✨ Key Features |
|:-------:|:--------------|:----------------|
| 🏠 **HOME** | Immersive video hero experience | Video background, animated typography, smooth scrolling |
| 🎨 **PROJECTS** | Dynamic portfolio showcase | Grid layout, hover effects, staggered animations |
| 👥 **AGENCY** | Interactive team presentation | Image sequences, scroll-based reveals, parallax |
| 📧 **CONTACT** | Engaging contact interface | Rotating elements, interactive design, smooth transitions |

</div>

<br />

<div align="center">
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" alt="line">
</div>

<br />

## 🛠️ Tech Stack

<div align="center">

### ⚡ Built With Modern Technologies

<br />

<p align="center">
  <img src="https://skillicons.dev/icons?i=react" width="48" height="48" alt="React" title="React 19" />&nbsp;&nbsp;&nbsp;
  <img src="https://skillicons.dev/icons?i=vite" width="48" height="48" alt="Vite" title="Vite 7" />&nbsp;&nbsp;&nbsp;
  <img src="https://skillicons.dev/icons?i=tailwind" width="48" height="48" alt="Tailwind" title="Tailwind CSS" />&nbsp;&nbsp;&nbsp;
  <img src="https://skillicons.dev/icons?i=javascript" width="48" height="48" alt="JavaScript" title="JavaScript" />&nbsp;&nbsp;&nbsp;
  <img src="https://skillicons.dev/icons?i=html" width="48" height="48" alt="HTML5" title="HTML5" />&nbsp;&nbsp;&nbsp;
  <img src="https://cdn.worldvectorlogo.com/logos/gsap-greensock.svg" width="48" height="48" alt="GSAP" title="GSAP" />
</p>

<br />

| Technology | Version | Purpose |
|:-----------|:-------:|:--------|
| ⚛️ **React** | `19.1.1` | UI Framework & Component Architecture |
| ⚡ **Vite** | `7.1.7` | Lightning-Fast Build Tool & Dev Server |
| 🎨 **Tailwind CSS** | `4.1.14` | Utility-First CSS Framework |
| 🎬 **GSAP** | `3.13.0` | Professional Animation Library |
| 🧭 **React Router** | `7.9.4` | Single Page App Routing |
| 🖼️ **Sharp** | `0.34.4` | High-Performance Image Processing |

</div>

<br />

<div align="center">
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" alt="line">
</div>

<br />

## 🚀 Quick Start

<div align="center">

### **Get up and running in 3 simple steps!**

</div>

<br />

```bash
# 📥 Step 1: Clone the repository
git clone https://github.com/ParthPipermintwala/k72-react-gsap.git
cd k72-react-gsap

# 📦 Step 2: Install dependencies
npm install

# 🚀 Step 3: Launch development server
npm run dev
```

<div align="center">

**🎉 That's it! Open http://localhost:5173 in your browser**

</div>

<br />

### 📋 **Available Commands**

<div align="center">

| 🎮 Command | 📝 Description | 🎯 Usage |
|:-----------|:--------------|:--------|
| `npm run dev` | 🔥 Start dev server with HMR | Development |
| `npm run build` | 📦 Create production build | Production |
| `npm run preview` | 👀 Preview production locally | Testing |
| `npm run lint` | 🔍 Run code quality checks | Code Quality |

</div>

<br />

<div align="center">
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" alt="line">
</div>

<br />

## 🎨 Showcase

<div align="center">

### ✨ Animation Arsenal

</div>

<br />

```
🎬  STAIR TRANSITIONS
    Elegant sequential page transitions with GSAP Timeline

📜  SCROLL TRIGGERS  
    Dynamic parallax and reveal effects using ScrollTrigger

🎞️  IMAGE SEQUENCES
    Frame-by-frame animations synchronized with scroll

🃏  CARD STAGGER
    Cascading entrance with staggered timing effects

🔍  SMOOTH SCALING
    Buttery zoom transitions with GSAP Tweens

✨  HOVER EFFECTS
    Interactive micro-interactions on user engagement
```

<br />

<div align="center">
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" alt="line">
</div>

<br />

## 📁 Structure

<div align="center">

### 🗂️ Project Organization

</div>

```
📦 k72/
┃
┣ 📂 public/
┃ ┗ 📁 fonts/                         🔤 Custom typography files
┃
┣ 📂 src/
┃ ┃
┃ ┣ 📁 assets/
┃ ┃ ┣ 📁 Photos/                      🖼️  Image Resources
┃ ┃ ┃ ┣ 📁 Agence_Photos/             👥 Team & agency imagery
┃ ┃ ┃ ┣ 📁 FullNavigation_Photos/     🧭 Navigation visuals
┃ ┃ ┃ ┗ 📁 Project_Photos/            🎨 Portfolio showcase
┃ ┃ ┗ 📁 Video/                       🎥 Video content
┃ ┃
┃ ┣ 📁 components/
┃ ┃ ┣ 📁 Agence/                      👥 Agency page components
┃ ┃ ┃ ┣ 📄 HeroSection.jsx            ✨ Agency hero with sequences
┃ ┃ ┃ ┗ 📄 Post.jsx                   👤 Team member cards
┃ ┃ ┃
┃ ┃ ┣ 📁 common/                      ♻️  Shared components
┃ ┃ ┃ ┣ 📄 Close.jsx                  ✖️ Close button
┃ ┃ ┃ ┣ 📄 Footer.jsx                 🔻 Footer section
┃ ┃ ┃ ┣ 📄 HamburgerLogo.jsx          ☰ Menu toggle
┃ ┃ ┃ ┣ 📄 Loading.jsx                ⏳ Loading animation
┃ ┃ ┃ ┣ 📄 Logo.jsx                   🏢 Brand logo
┃ ┃ ┃ ┗ 📄 Stair.jsx                  🎬 Page transitions
┃ ┃ ┃
┃ ┃ ┣ 📁 Contact/                     📧 Contact components
┃ ┃ ┃ ┣ 📄 Icon.jsx                   🎯 Decorative icons
┃ ┃ ┃ ┗ 📄 TalkAbout.jsx              💬 Contact sections
┃ ┃ ┃
┃ ┃ ┣ 📁 home/                        🏠 Home page components
┃ ┃ ┃ ┣ 📄 HomeBottomText.jsx         📝 Bottom content
┃ ┃ ┃ ┣ 📄 HomeHeroText.jsx           💫 Hero typography
┃ ┃ ┃ ┗ 📄 Video.jsx                  🎬 Background video
┃ ┃ ┃
┃ ┃ ┣ 📁 Navigation/                  🧭 Navigation system
┃ ┃ ┃ ┣ 📄 FullScreenNav.jsx          🖥️ Full menu overlay
┃ ┃ ┃ ┣ 📄 MenuContainer.jsx          📦 Menu wrapper
┃ ┃ ┃ ┣ 📄 Navbar.jsx                 🔝 Main navbar
┃ ┃ ┃ ┗ 📄 NavBarLink.jsx             🔗 Nav links
┃ ┃ ┃
┃ ┃ ┗ 📁 projects/                    🎨 Project components
┃ ┃   ┣ 📄 Card.jsx                   🃏 Project cards
┃ ┃   ┗ 📄 CardContainer.jsx          📋 Card grid
┃ ┃
┃ ┣ 📁 context/
┃ ┃ ┗ 📄 NavContext.jsx                🔄 Navigation state
┃ ┃
┃ ┣ 📁 pages/                          📄 Route Pages
┃ ┃ ┣ 📄 Home.jsx                      🏠 Landing page
┃ ┃ ┣ 📄 Project.jsx                   🎨 Portfolio page
┃ ┃ ┣ 📄 Agence.jsx                    👥 Agency page
┃ ┃ ┗ 📄 Contact.jsx                   📧 Contact page
┃ ┃
┃ ┣ 📄 App.jsx                         ⚛️ Root component
┃ ┣ 📄 main.jsx                        🚀 Application entry
┃ ┗ 📄 index.css                       🎨 Global styles
┃
┣ 📄 index.html                        📄 HTML template
┣ 📄 vite.config.js                    ⚙️ Vite configuration
┣ 📄 eslint.config.js                  ✅ Linting rules
┣ 📄 package.json                      📦 Dependencies
┗ 📄 README.md                         📖 Documentation
```

<br />

<div align="center">
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" alt="line">
</div>

<br />

## 🌐 Browser Support

<div align="center">

<br />

<p align="center">
  <img src="https://raw.githubusercontent.com/alrra/browser-logos/master/src/chrome/chrome_48x48.png" width="48" height="48" alt="Chrome" title="Chrome - Latest" />&nbsp;&nbsp;&nbsp;&nbsp;
  <img src="https://raw.githubusercontent.com/alrra/browser-logos/master/src/firefox/firefox_48x48.png" width="48" height="48" alt="Firefox" title="Firefox - Latest" />&nbsp;&nbsp;&nbsp;&nbsp;
  <img src="https://raw.githubusercontent.com/alrra/browser-logos/master/src/safari/safari_48x48.png" width="48" height="48" alt="Safari" title="Safari - Latest" />&nbsp;&nbsp;&nbsp;&nbsp;
  <img src="https://raw.githubusercontent.com/alrra/browser-logos/master/src/edge/edge_48x48.png" width="48" height="48" alt="Edge" title="Edge - Latest" />
</p>

<p align="center">
  <strong>✅ All latest versions supported</strong>
</p>

<br />

<p>
  <img src="https://img.shields.io/badge/Requires-ES6+-yellow?style=flat-square" alt="ES6+" />
  <img src="https://img.shields.io/badge/JavaScript-Enabled-green?style=flat-square" alt="JS Enabled" />
</p>

</div>

<br />

<div align="center">
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" alt="line">
</div>

<br />

## 📂 Asset Guidelines

<div align="center">

| 🎯 Type | 📍 Location | 🎨 Format | 📝 Notes |
|:-------:|:-----------|:----------|:---------|
| 🖼️ **Images** | `src/assets/Photos/` | `.webp` `.jpg` `.png` | Optimized & compressed |
| 🎥 **Videos** | `src/assets/Video/` | `.mp4` `.webm` | Use MP4 for compatibility |
| 🔤 **Fonts** | `public/fonts/` | `.woff` `.woff2` | Modern font formats |

</div>

<br />

<div align="center">
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" alt="line">
</div>

<br />

<div align="center">

## 👨‍💻 About

<br />

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=20&duration=3000&pause=1000&color=D3FD50&center=true&vCenter=true&multiline=false&repeat=true&width=600&lines=Created+by+Parth+Pipermintwala" alt="Author" />

<br />

<p>
  <a href="https://github.com/ParthPipermintwala/k72-react-gsap">
    <img src="https://img.shields.io/badge/Portfolio-K72-D3FD50?style=for-the-badge&logo=google-chrome&logoColor=white" alt="Portfolio" />
  </a>
  <a href="https://www.linkedin.com/in/parth-pipermintwala-403a82367/">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
  <a href="https://github.com/ParthPipermintwala">
    <img src="https://img.shields.io/badge/GitHub-Follow-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
  </a>
</p>

<br />

### 📊 Project Stats

<p>
  <img src="https://img.shields.io/badge/Components-25+-blue?style=flat-square" alt="Components" />
  <img src="https://img.shields.io/badge/Animations-50+-green?style=flat-square" alt="Animations" />
  <img src="https://img.shields.io/badge/Pages-4-orange?style=flat-square" alt="Pages" />
  <img src="https://img.shields.io/badge/Bundle_Size-Optimized-success?style=flat-square" alt="Bundle" />
</p>

<br />

---

<br />

### 📝 License

<p>
  <img src="https://img.shields.io/badge/License-MIT-green?style=for-the-badge" alt="License" />
  <br />
  <sub><strong>MIT License</strong> - Free to use and modify</sub>
</p>

<br />

---

<br />

<h3>
  Built with ❤️ using React • GSAP • Modern Web Technologies
</h3>

<br />

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=100&section=footer" alt="Footer" />

</div>
